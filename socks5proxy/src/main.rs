...
fn handle_client(src_stream: &TcpStream, id: u64) -> Result<(), Box<dyn std::error::Error>> {
    ...
    thread::spawn(move || {
        copy(&mut src_reader, &mut dst_writer).ok();
    });
    copy(&mut dst_reader, &mut src_writer).ok();
...
fn main() {
    let args = Args::parse();
    println!("Listening and server on {}", args.listen);
    ...
    for stream in listener.incoming() {
        match stream {
            Ok(stream) => {
                id += 1;
                thread::spawn(move || {
                    if let Err(err) = handle_client(&stream, id) {
                        println!("error: {:?}", err);
                    }
                });
            }
